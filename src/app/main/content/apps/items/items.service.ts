...
export class ItemsService implements Resolve<any> {
  ...
  constructor(private http: HttpClient, private authService: AuthService,  private helpersService: HelpersService,
  ...
  getItems(): Promise<any> {
    return new Promise((resolve, reject) => {
        this.http.get<Item[]>(AppConfig.apiUrl + 'items?access_token=' + this.authService.getToken())
  ...
  getItemsCount(): Promise<any> {
    return new Promise((resolve, reject) => {
        this.http.get<Item[]>(AppConfig.apiUrl + 'items/count?access_token=' + this.authService.getToken())
          .subscribe((response: any) => {
  ...
  deleteItem(item): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.http.delete<Item>(AppConfig.apiUrl + 'items/' + item.id + '?access_token=' + this.authService.getToken())
        .subscribe(
          data => {
            console.log(data);
  ...
  viewItem(itemId: string): Promise<any> {
    return new Promise((resolve, reject) => {
      // send get request
      this.http.get<Item>(AppConfig.apiUrl+'items/' + itemId+'?filter[include]=owner&filter[include]=product&access_token=' + this.authService.getToken())
        .subscribe(
          item => {
  ...
  editItem(item: Item): Promise<any> {
    return new Promise((resolve, reject) => {
      this.http.patch<Item>(AppConfig.apiUrl+'items/' + item.id + '?access_token=' + this.authService.getToken(), item)
        .subscribe(
          data => {
            resolve(true);
          },
  ...
  newItem(item: Item): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.http.post<Item>(AppConfig.apiUrl + 'items/?access_token=' + this.authService.getToken(), item)
        .subscribe(
          data => {
            console.log(data);
            resolve(true);
